    ...
    def __call__(
        ...
        if self.has_variable("cache", "cached_key") or init_cache:
            query, key, value = self.attn.forward_qkv(
                hidden_states, position_ids, deterministic
            )
            ...
            attn_output = self.attn.attn_out_proj(
                attn_output, deterministic=deterministic
            )
            ffn_output = self.attn.forward_ffn(
                hidden_states + attn_output, deterministic=deterministic
            )
...
def blockwise_compute(
    ...
    q_inputs, kv_inputs = map(
        lambda t: rearrange(t, "b n c d -> n b c d"), (q_inputs, kv_inputs)
    )
    ...
    def _query_chunk_attention(cell, _, args):
        ...
        def summarize_chunk(cell, carry, args):
            ...
            key_chunk, value_chunk = cell.forward_key_value(
                kv_chunk, kv_position_ids_chunk
            )
        ...
        init_carry = Carry(
            jnp.zeros(
                (batch, query_chunk_size, num_heads, dim_per_head), dtype=q_inputs.dtype
            ),
            jnp.zeros(
                (batch, query_chunk_size, num_heads, dim_per_head), dtype=q_inputs.dtype
            ),
            (-jnp.inf)
            * jnp.ones((batch, query_chunk_size, num_heads, 1), dtype=q_inputs.dtype),
        )
...
def test_bpt():
    ...
    block = AttentionBlock(
        ...
        hidden_size=config_13b['n_embd'],
        num_heads=config_13b['n_head'],
        rotary_dim=config_13b['rotary_dim'],
        intermediate_size=4 * config_13b['n_embd'], # Assuming 4x hidden_size for intermediate size.
        max_position_embeddings=config_13b['n_positions']
    )
    block.apply(
        jnp.ones(
            (
                1, 
                config_13b['n_positions'],
                config_13b['n_embd']
            )
        ),
        jnp.ones(
